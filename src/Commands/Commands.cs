...
namespace XcTools.Commands
{
    public class Commands
    {
        ...
        // 卸载自动加载登记：从 acad.lsp 移除本插件写入的 (load) 行，
        // 同时移除菜单栏显示。重启后不再自动加载。
        [CommandMethod("XCTUNSET", CommandFlags.Modal)]
        public void UnsetAutoLoader()
        {
            try
            {
                Editor ed = AcApp.DocumentManager.MdiActiveDocument.Editor;
                ...
                int files = App.UnregisterAcadLspLoader();

                // 同步移除当前会话菜单栏显示
                try { CuixManager.UnloadMenu(); }
                catch (System.Exception ex) { App.Log($"XCTUNSET: unload menu failed: {ex.Message}"); }

                if (files > 0)
                    ed.WriteMessage($"\n已清理 {files} 个 acad.lsp 文件，下次启动 AutoCAD 将不再自动加载小辰CAD工具箱。");
                else
                    ed.WriteMessage("\n未发现小辰CAD工具箱的自动加载登记（可能已清理过）。");
                ed.WriteMessage("\n如需完全卸载，请关闭 AutoCAD 后删除插件文件夹。");
            }
            catch (System.Exception ex)
            {
                AcApp.DocumentManager.MdiActiveDocument.Editor.WriteMessage($"\n卸载自动加载错误: {ex.Message}");
